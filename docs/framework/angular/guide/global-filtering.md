...
```ts
readonly globalFilter = signal("search term") //recommended to initialize globalFilter state here
...
readonly table = injectTable(() => ({
  ...
  state: {
    globalFilter: this.globalFilter(), // pass our managed globalFilter state to the table
  },
})
```
